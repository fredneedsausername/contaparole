...
int contaparole_conta_parola(alberobinario_NodoBinario* root, char* parola) {
    alberobinario_NodoBinario* nodoBinario = alberobinario_contenuto_cerca(root, parola, contaparole_funzione_comparazione_parola);
...
void contaparole_inizializza_albero(alberobinario_NodoBinario* root, const char* parola) {
    alberobinario_nodo_inizializza(root);
    root->contenuto = malloc(sizeof(contaparole_Parola));
    ((contaparole_Parola*)(root->contenuto))->parola = _strdup(parola);
...
void contaparole_loop_applicazione() {
    ...
    char nomeFileInput[MAX_FILE_NAME_LENGTH];
    char nomeFileOutput[MAX_FILE_NAME_LENGTH];
    printf("Nome file input: ");
    fgets(nomeFileInput, sizeof(nomeFileInput), stdin);
    printf("Nome file output: ");
    fgets(nomeFileOutput, sizeof(nomeFileOutput), stdin);
    if (nomeFileInput[strlen(nomeFileInput) - 1] == '\n') nomeFileInput[strlen(nomeFileInput) - 1] = '\0';
    if (nomeFileOutput[strlen(nomeFileOutput) - 1] == '\n') nomeFileOutput[strlen(nomeFileOutput) - 1] = '\0';
    FILE* input;
    fopen_s(&input, nomeFileInput, "r");
    fopen_s(&output, nomeFileOutput, "w");
